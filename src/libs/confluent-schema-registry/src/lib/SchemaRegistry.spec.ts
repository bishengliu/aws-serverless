...
const personSchema = readAVSC(
  path.join(__dirname, '../fixtures/avsc/person.avsc')
);
...
describe('SchemaRegistry', () => {
  ...
  describe('#register', () => {
    ...
    it('uploads the new schema', async () => {
      await expect(
        api.Subject.latestVersion({ subject })
      ).rejects.toHaveProperty(
        'message',
        `${DEFAULT_API_CLIENT_ID} - Subject not found.`
      );

      await expect(schemaRegistry.register(Schema)).resolves.toEqual({
    ...
    it('sets the compatibility according to param', async () => {
      ...
      const response = await api.Subject.config({ subject });
      expect(response.data()).toEqual({
        compatibilityLevel: COMPATIBILITY.NONE,
      });
    });
    ...
    it('accepts schema without a namespace when subject is specified', async () => {
      delete Schema.namespace;
      const nonNamespaced = readAVSC(
        path.join(__dirname, '../fixtures/avsc/non_namespaced.avsc')
      );
      await expect(
        schemaRegistry.register(nonNamespaced, { subject })
      ).resolves.toEqual({
        id: expect.any(Number),
      });
    });
    ...
    it('throws an error when the configured compatibility is different than defined in the client', async () => {
      await schemaRegistry.register(Schema);
      await api.Subject.updateConfig({
        subject,
        body: { compatibility: COMPATIBILITY.FULL },
      });
      await expect(schemaRegistry.register(Schema)).rejects.toHaveProperty(
        'message',
  ...
  describe('#encode', () => {
    ...
    it('throws an error if registryId is empty', async () => {
      await expect(
        schemaRegistry.encode(undefined, payload)
      ).rejects.toHaveProperty('message', 'Invalid registryId: undefined');
    });
  ...
  describe('#decode', () => {
    ...
    it('decodes data', async () => {
      const buffer = Buffer.from(
        await schemaRegistry.encode(registryId, payload)
      );
  ...
  describe('#getRegistryIdBySchema', () => {
    ...
    it('returns the registry id if the schema has already been registered under that subject', async () => {
      ...
      await expect(
        schemaRegistry.getRegistryIdBySchema(subject, Schema)
      ).resolves.toEqual(id);
    });

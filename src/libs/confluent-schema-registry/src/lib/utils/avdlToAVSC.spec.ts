...
const compareWithJavaImplementation = (
  avdlPath: string,
  name: string
) => async () => {
...
test(
  'protocol with two levels',
  compareWithJavaImplementation('two.avdl', 'Two')
);

test(
  'protocol with multiple levels',
  compareWithJavaImplementation('multiple.avdl', 'Multiple')
);

test(
  'protocol with union',
  compareWithJavaImplementation('union.avdl', 'Union')
);
...
test(
  'protocol with enum & union',
  compareWithJavaImplementation('enum_union.avdl', 'EnumUnion')
);

test(
  'protocol with array',
  compareWithJavaImplementation('array.avdl', 'Array')
);

test(
  'protocol with array & union',
  compareWithJavaImplementation('array_union.avdl', 'ArrayUnion')
);

test(
  'protocol with really complex stuff',
  compareWithJavaImplementation('complex.avdl', 'Complex')
);

test(
  'protocol with multiple namespaces',
  compareWithJavaImplementation(
    'multiple_namespaces.avdl',
    'MultipleNamespaces'
  )
);
...
test(
  ...
  compareWithJavaImplementation(
    'import_multiple_namespaces.avdl',
    'ImportMultipleNamespaces'
  )
);

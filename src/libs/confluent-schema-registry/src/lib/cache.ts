...
import { ForSchemaOptions, types } from 'avsc';
...
export default class Cache {
  ...
  private schemaKeyGen = ({ subject, version }: SchemaRef): string =>
    `${subject}:${version}`;
  ...
  getLatestRegistryId = (subject: string): number | undefined =>
    this.registryIdBySubject[subject];
  ...
  getSchema = (registryId: number): Schema =>
    this.schemasByRegistryId[registryId];

...
// We allow ignore ts errors/warnings in this file because it was not created by us.
...
export default class SchemaRegistry {
  ...
  public async register(
    ...
    if (userOpts && userOpts.subject) {
      ...
      if (!schema.namespace) {
        throw new ConfluentSchemaRegistryArgumentError(
          `Invalid namespace: ${schema.namespace}`
        );
      }
    ...
    try {
      ...
      const {
        compatibilityLevel,
      }: { compatibilityLevel: COMPATIBILITY } = response.data();
      ...
      logicalTypes = Object.fromEntries(
        await Promise.all(
          foundSchema.references.map(async ({ name, subject, version }) => {
            const schemaType = await this.getSchema(
              await this.getRegistryId(subject, version)
            );
  ...
  public async decode(buffer: Buffer): Promise<any> {
    ...
    if (Buffer.compare(MAGIC_BYTE, magicByte) !== 0) {
      throw new ConfluentSchemaRegistryArgumentError(
        `Message encoded with magic byte ${JSON.stringify(
          magicByte
        )}, expected ${JSON.stringify(MAGIC_BYTE)}`
      );
    }
    const schema = await this.getSchema(registryId);
